...
const updatePostVariantOfPost = async (id, data) => {
  // if (files.length === 0) {
  const response = await axios.put(`${process.env.REACT_APP_API}/post-variants/${id}`, data)
  return response.data;
  // }
  ...
  // const response = await axios.put(`${process.env.REACT_APP_API}/post-variants/${id}`, formData, { formData: true })
  // return response.data;
};


export { createPostVariantOfPost, deletePostVariantOfPost, getAllPostVariantOfPostByParentId, updatePostVariantOfPost };